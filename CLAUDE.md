...
主對話是指揮官。找/查/追（探索）→ 派 `Explore`；改/跑/修、做了要跑東西才知道對不對（執行）→ 派 agent；定/寫 brief/審結論/答使用者（運籌）→ 自己。Read 只用於使用者指定的段落、抽查回報引用的 檔案:行號、運籌素材原文（計畫/findings/交付物，不含 source code）；用 Read 去「找」就是探索。唯一例外：無回饋迴圈的一次 Edit。多任務（≥2 件彼此獨立、各需一輪執行的事）：先列全清單 → 同訊息並行派出 → 全部完成才結束 turn。可完整腳本化的批次轉換 → 寫腳本跑，不派 agent。
...
- 開發新功能 → /openspec-explore 先釐清需求再動工；code review → 原生 /code-review。
